...
let score = 0;
let incorrectAnswers = []; // Noto'g'ri javoblarni saqlash uchun
...
document.getElementById('submitQuiz').addEventListener('click', function() {
    ...
    if (!phoneInput) {
        alert("Phone number is required to start the quiz.");
        return;
    }
    ...
    questions.forEach((q, index) => {
        ...
        if (userAnswer.toLowerCase() === q.answer.toLowerCase()) {
            ...
            incorrectAnswers.push({ question: q.question, userAnswer: userAnswer, correctAnswer: q.answer });
        }
    });

    const name = localStorage.getItem('name');
    alert(`${name}, you scored ${score} out of ${questions.length}`);
    sendResultsToBot(name, phoneInput, score, incorrectAnswers);
});

function sendResultsToBot(name, phone, score, incorrectAnswers) {
    const botToken = '7818104445:AAFZz4_kbDvzEaBWDnf7qupPY1EHsJL7xHE';
    const chatId = '6206221012'; // O'zgartiring
    ...
    if (incorrectAnswers.length > 0) {
        ...
        message += "No incorrect answers. Great job!";
    }
